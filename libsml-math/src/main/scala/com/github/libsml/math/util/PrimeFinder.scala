...
/**
 * Created by huangyu on 15/7/19.
 */
object PrimeFinder {
  /** The largest prime this class can generate; currently equal to <tt>Integer.MAX_VALUE</tt>. */
  val LARGEST_PRIME: Int = Integer.MAX_VALUE
  /**
   ...
   */
  private val PRIME_CAPACITIES: Array[Int] = Array(LARGEST_PRIME,
  ...
  util.Arrays.sort(PRIME_CAPACITIES);
  ...
  def nextPrime(desiredCapacity: Int): Int = {
    var i: Int = java.util.Arrays.binarySearch(PRIME_CAPACITIES, desiredCapacity)
    if (i < 0) {
      i = -i - 1
    }
